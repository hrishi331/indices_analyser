...
# Header
st.header("Intraday Supports".upper(),divider='gray')
...
if st.button(label='SUBMIT'):
    ...
    data = round(df['LOW']/n[timeframe])*n[timeframe]
    
    resistance_table = data.value_counts().reset_index().sort_values(by='count')
    today_price = df['CLOSE'].iloc[-1]

    resistance_table = resistance_table[resistance_table['LOW']<today_price]
    ...
    st.write(resistance_table.set_index('LOW'))
